    ...
    public init(subsystem: String, category: String) {
        ...
        if Self.fileHandle == nil {
            ...
            if let docsDir = fm.urls(for: .documentDirectory, in: .userDomainMask).first {
                ...
                if let allZipFiles = try? fm.subpathsOfDirectory(atPath: logsDir.path).filter({
                    $0.hasSuffix(".\(subsystem).log.zip")
                }),
                ...
                if let allLogFiles = try? fm.subpathsOfDirectory(atPath: docsDir.path).filter({
                    $0.hasSuffix(".\(subsystem).log")
                }) {
                    for logFile in allLogFiles {
                        let fileUrl = docsDir.appendingPathComponent(logFile)
                        let zipFileUrl = logsDir.appendingPathComponent(logFile)
                        ...
                        do {
                            ...
                            print(
                                "Failed to zip log file '\(fileUrl.path)': \(error.localizedDescription)"
                            )
                        }
                    }
                }
                ...
                let logFilePath = docsDir.appendingPathComponent(baseFileName)
                    .appendingPathExtension("\(subsystem).log").path
